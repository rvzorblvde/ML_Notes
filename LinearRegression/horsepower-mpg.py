...
plt.scatter(auto_simple["horsepower"], auto_simple["mpg"], alpha=0.65)
plt.xlabel("Caballos de Fuerza")
plt.ylabel("Millas por Galón")
plt.title(f"Auto: Potencia (Caballos de Fuerza) vs MPG")
plt.grid(True)
plt.show()

# 5. Entrenar el modelo
X = auto_simple[["horsepower"]]
y = auto_simple["mpg"]
...
x_linea = np.linspace(X["horsepower"].min(), X["horsepower"].max(), 200).reshape(-1, 1)
y_linea = modelo_mpg.predict(x_linea)

plt.scatter(X["horsepower"], y, alpha=0.65, label="Datos")
plt.plot(x_linea, y_linea, linewidth=2, color='red', label='Recta de Regresión')
plt.xlabel("Caballos de Fuerza")
plt.ylabel("Millas Por Galón")
plt.title(f"Regresión Lineal Simple: Caballos de Fuerza → MPG")
plt.legend()
plt.grid(True)
plt.show()
...
# Predicción
auto_nuevo = pd.DataFrame({"horsepower": [100]})
